...
try:
    ...
    # Streamlit 앱은 이 모듈을 import하는 순간 죽으면 안 된다.
    # python-pptx/lxml이 아직 설치되지 않은 venv에서는 RGBColor가 없으므로
    # 색상 상수도 안전하게 지연 생성하고, PPTX 생성 시점에 명확한 오류를 낸다.
...
def _rgb(r: int, g: int, b: int):
    if RGBColor is None:  # type: ignore[name-defined]
        return None
...
NAVY = _rgb(17, 24, 39)
GRAY = _rgb(75, 85, 99)
LIGHT = _rgb(248, 250, 252)
BORDER = _rgb(229, 231, 235)
WHITE = _rgb(255, 255, 255)
GREEN = _rgb(16, 185, 129)
ORANGE = _rgb(245, 158, 11)
RED = _rgb(239, 68, 68)
...
def _require_pptx() -> None:
    if Presentation is None:
        raise RuntimeError(
            ...
            "로컬에서는 `pip install -r requirements.txt` 또는 "
            "`pip install python-pptx`를 실행한 뒤 Streamlit을 완전히 재시작하세요. "
            "Streamlit Cloud에서는 requirements.txt와 packages.txt가 GitHub에 올라갔는지 확인하세요. "
            f"원래 import 오류: {_IMPORT_ERROR}"
        )


def _set_font(run, size: int = 18, bold: bool = False, color=None) -> None:
    run.font.name = "맑은 고딕"
...
def _align(name: str):
    """python-pptx가 import되지 않은 상태에서도 모듈 import가 깨지지 않도록 정렬값을 지연 조회한다."""
    if PP_ALIGN is None:
        return None
    return getattr(PP_ALIGN, name)


def _add_textbox(slide, x, y, w, h, text: str = "", font_size: int = 18, bold: bool = False, color=None, align=None):
...
def _add_card(slide, x, y, w, h, title: str, items: list[str], title_color=None, fill=None):
...
def _add_footer(slide, text: str = "Galaxy VOC Collector · AI/RAG 기반 자동 생성 보고서"):
    _add_textbox(slide, Inches(0.55), Inches(7.05), Inches(12.0), Inches(0.25), text, 9, False, GRAY, _align("RIGHT"))
...
def _title_slide(prs, title: str, subtitle: str, meta: str):
    ...
    _add_textbox(slide, Inches(1.05), Inches(1.08), Inches(11.1), Inches(0.7), title, 30, True, WHITE, _align("CENTER"))
    _add_textbox(slide, Inches(1.2), Inches(2.75), Inches(10.9), Inches(0.8), subtitle, 20, False, NAVY, _align("CENTER"))
    ...
    _add_textbox(slide, Inches(1.2), Inches(5.85), Inches(10.9), Inches(0.45), meta, 12, False, GRAY, _align("CENTER"))
    return slide
...
def _metrics_slide(prs, stats: dict, rag_data: dict | None = None):
    ...
    for i, (label, value, color) in enumerate(metric_values):
        ...
        _add_textbox(slide, x + Inches(0.08), Inches(1.62), Inches(2.55), Inches(0.3), label, 12, True, GRAY, _align("CENTER"))
        _add_textbox(slide, x + Inches(0.08), Inches(1.95), Inches(2.55), Inches(0.5), value, 24, True, color, _align("CENTER"))
