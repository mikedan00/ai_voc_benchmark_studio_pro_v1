...
def generate_markdown_docx(
    markdown_text: str,
    *,
    title: str = "VOC 요약·인사이트 리포트",
    output_dir: str = "output",
    file_prefix: str = "VOC_Insight_Report",
) -> str:
    """Markdown 텍스트를 최소 DOCX로 저장한다. python-docx 없이 Streamlit Cloud에서 동작한다."""
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    safe_prefix = re.sub(r"[^0-9A-Za-z가-힣_.-]+", "_", file_prefix).strip("_") or "VOC_Insight_Report"
    path = out / f"{safe_prefix}_{time.strftime('%Y%m%d_%H%M%S')}.docx"
    paragraphs: list[str] = []
    paragraphs.append(_heading(title, 1))
    paragraphs.append(_p(f"생성일: {time.strftime('%Y-%m-%d %H:%M:%S')}"))
    paragraphs.append(_p(""))
    for raw in (markdown_text or "").splitlines():
        ...
        if not line:
            ...
            continue
        if line.startswith("# "):
            paragraphs.append(_heading(line[2:].strip(), 1))
        elif line.startswith("## "):
            paragraphs.append(_heading(line[3:].strip(), 2))
        elif line.startswith("### "):
            paragraphs.append(_heading(line[4:].strip(), 3))
        elif line.lstrip().startswith(('- ', '* ')):
            paragraphs.append(_bullet(line.lstrip()[2:].strip()))
        elif re.match(r"^\d+\.\s+", line.strip()):
            paragraphs.append(_bullet(line.strip()))
        elif line.startswith(">"):
            paragraphs.append(_p("인용: " + line.lstrip("> ").strip()))
        elif line.startswith("```"):
            continue
        else:
            paragraphs.append(_p(re.sub(r"\*\*(.*?)\*\*", r"\1", line)))
    _write_docx(path, paragraphs)
    return str(path)
